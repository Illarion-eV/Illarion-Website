...
LEFT JOIN accounts.account AS account_proof_reader_de ON news_proof_reader_de_id = account_author_de.acc_id
LEFT JOIN accounts.account AS account_proof_reader_us ON news_proof_reader_us_id = account_author_de.acc_id
WHERE news_id = {$pgSQL->Quote($id)}
SQL;
        $pgSQL->setQuery($query, 0, 1);
