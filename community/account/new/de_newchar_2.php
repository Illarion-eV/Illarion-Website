...
<div>
	...
	<form action="<?php echo Page::getURL(); ?>/community/account/new/de_newchar.php?charid=<?php echo $charid,($_GET['server'] == '1' ? '&amp;server=1' : ''); ?>" method="post" name="create_char" id="create_char">
		<div>
			...
			<img id="char_image" src="<?php echo char_create::getConvertedImageUrl(char_create::getImageName($race, $sex),substr($skincolors[0],1)); ?>" style="position:relative;left:133px; top:73px;display:block;margin-bottom:-100px;" />
			...
			<img id="hair_image" src="<?php echo char_create::getConvertedImageUrl(char_create::getImageName($race, $sex).$hairvalues[1],substr($haircolors[0],1)); ?>" style="display:block;position:relative;left:133px; top:73px;margin-bottom:-100px;" />
			<img id="beard_image" src="<?php echo char_create::getConvertedImageUrl(substr(char_create::getImageName($race, $sex), 0, -1)."m".$beardvalues[0],substr($haircolors[0],1)); ?>" style="position:relative;left:133px; top:73px;" />
			</div>
				...
				{ ?>
					...
					<select name="beard" id="beard" onchange="hairChange('<?php echo char_create::getImageName($race, $sex); ?>', h_color)" style="width:100%;">
						<?php foreach( $beardvalues as $key => $beard ): ?>
							<option value="<?php echo $key; ?>"
							<?php if ($key == 0) { echo ' selected="selected"'; } ?>
