	...
	<form action="<?php echo Page::getURL(); ?>/community/account/new/de_newchar.php?charid=<?php echo $charid,($_GET['server'] == '1' ? '&amp;server=1' : ''); ?>" method="post" name="create_char" id="create_char">
		<div>
			...
			<div style="height:250px;padding-left:320px;padding-right:20px;">
                ...
                <a onclick="hairChange('<?php echo char_create::getImageName($race, $sex) ?>'+h_value, '<?php echo substr($color, 1); ?>')" style="display: block;height: 10px;width: 10px;float: left;background-color: <?php echo $color; ?>;border: 1px solid black;"></a>
                <?php endforeach; ?>
				Haare:
				<input type="hidden" id="hairvalue" value="" name="hairvalue" />
				<select name="hair" onchange="hairChange('<?php echo char_create::getImageName($race, $sex) ?>'+this.options[this.selectedIndex].value, h_color)" style="width:100%;">
