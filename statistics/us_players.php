...
<h2>
	<span class="float_left">Overview (State: <?php echo strftime( '%d. %B %Y %H:%I%p', IllaDateTime::TimestampWithOffset() ) ?>)</span>
		...
		foreach($list as $key=>$char)
		{
			...
			if ($show_char)
			{
				if ($show_profil)
				{
					...
					$content[$current_list] .= '<a class="rating'.$chr_settings[$char['chr_playerid']]['votes_result'].'" href="'.Page::getURL().'/community/us_charprofile.php?id='.dechex( $char['chr_playerid'] ).'">'.$char['chr_name'] . '</a>';
				}
				else
				{
			...
			else
			{
				if ($show_profil)
				{
					...
					$content[$current_list] .= '<a class="hidden rating'.$chr_settings[$char['chr_playerid']]['votes_result'].'" href="'.Page::getURL().'/community/us_charprofile.php?id='.dechex( $char['chr_playerid'] ).'">'.$char['chr_name'] . '</a>';
				}
				else
				{
